...
import hotel.management.system.imt.addroom.AddRoomController;
import hotel.management.system.imt.bookRoom.BookRoomController;
import hotel.management.system.imt.checkOut.CheckOutController;
...
public class MainController implements Initializable {
    ...
    private final ArrayList<String> invoice_data = new ArrayList<>();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        rooms_table.setOnMousePressed((MouseEvent event) -> {
            ...
            ArrayList<String> selectedRoomm = new ArrayList<>();
            if(multiSelection){
            selectedItems.forEach((row) -> {
                selectedRoomm.add(row.getRoomId()+","+row.getCustomerId()+","+row.getCustomerName()+","+row.getGuests()
                +","+row.getDateBooked()+","+row.getCheckIn()+","+row.getCheckOut()+","+row.getType());
                invoice_data.clear();
            });
            
            ListIterator<String> iterator = selectedRoomm.listIterator();
            while (iterator.hasNext()) {
        ...
        booked_rooms_table.setRowFactory((TableView<BookedRoom> tableView) -> {
            ...
            bookItem.setOnAction((ActionEvent event) -> {
               ...
               invoice_data.add(selectedBookedRoom.getRoomId()+","+selectedBookedRoom.getCustomerId()+","+selectedBookedRoom.getCustomerName()+","+selectedBookedRoom.getGuests()
                +","+selectedBookedRoom.getDateBooked()+","+selectedBookedRoom.getCheckIn()+","+selectedBookedRoom.getCheckOut()+","+selectedBookedRoom.getType());  
             }
                loadCheckOut("/hotel/management/system/imt/checkOut/CheckOut.fxml", "Check Out", invoice_data);
            });
            ...
            bookItem.setOnAction((ActionEvent event) -> {
                if(multiIds.equals(""))
                    loadBookRoom("/hotel/management/system/imt/bookRoom/BookRoom.fxml", "Book Room(s)", row.getItem().getId());
                else
                    loadBookRoom("/hotel/management/system/imt/bookRoom/BookRoom.fxml", "Book Room(s)", multiIds);
            });
            MenuItem editItem = new MenuItem("Edit");
            editItem.setOnAction((ActionEvent event) -> {
                if(rooms_table.getSelectionModel().getSelectedItem()!=null){
                    loadUpdateRoom("/hotel/management/system/imt/addroom/addRoom.fxml", "Add New Room", rooms_table.getSelectionModel().getSelectedItem());
                    }
            });
            MenuItem removeItem = new MenuItem("Delete");
            removeItem.setOnAction((ActionEvent event) -> {
    ...
    private void handleBookInButtonAction(ActionEvent event) {
        if(multiIds.equals(""))
            loadBookRoom("/hotel/management/system/imt/bookRoom/BookRoom.fxml", "Book Room(s)", selectedRoom.getId());
        else
        loadBookRoom("/hotel/management/system/imt/bookRoom/BookRoom.fxml", "Book Room(s)", multiIds);
        //loadWindow("/hotel/management/system/imt/bookRoom/BookRoom.fxml", "Book Room(s)");
    }
    ...
    private void handleCustomerInfoButtonAction(ActionEvent event) {
        loadWindow("/hotel/management/system/imt/addcustomer/addCustomer.fxml", "Customers Info");
    }

    @FXML
    private void handleSettingsButtonAction(ActionEvent event) {
        loadWindow("/hotel/management/system/imt/settings/Settings.fxml", "Admin Settings");
    }

    @FXML
    private void handleAddRoomMenuAction(ActionEvent event) {
         loadWindow("/hotel/management/system/imt/addroom/addRoom.fxml", "Add New Room");
    }
    ...
    private void handleAddRoomCategoryMenuAction(ActionEvent event) {
        loadWindow("/hotel/management/system/imt/addcategory/addCategory.fxml", "Room Category");
    }
    ...
    private void loadBookedRoomData() {
        ...
        try {
         ...
         String bookType=rs.getString("type");
                ...
                bookedRoomList.add(new BookedRoom(roomNo, customerId, customerName, Integer.toString(guest), booked_date, checkIn, checkOut, bookType));
    ...
    private void handleUpdateMenuAction(ActionEvent event) {
        if(selectedRoom!=null){
            loadUpdateRoom("/hotel/management/system/imt/addroom/addRoom.fxml", "Update Room", selectedRoom);
        }
    }
  ...
  return stage;
   ...
   private Stage loadCheckOut(String loc, String title, ArrayList<String> invoice_data) {
       try{
  FXMLLoader loader = new FXMLLoader(getClass().getResource( loc));
  Stage stage = new Stage(StageStyle.DECORATED);
  stage.setScene(new Scene((Pane) loader.load()));
